import { AfterContentInit, Attribute, ChangeDetectorRef, Component, ContentChildren, ElementRef, EventEmitter, Input, OnInit, Optional, Output, QueryList, ViewChild } from '@angular/core';
import { ControlValueAccessor, NG_VALUE_ACCESSOR } from '@angular/forms';

import { interval, map, Observable, shareReplay, takeUntil } from 'rxjs';
...
export class SelectComponent extends SubscriptionHandler implements OnInit, AfterContentInit, ControlValueAccessor {
    ...
    id: string;
    containerTransform: number = 0;
    optionsContainerScroll: number = 0;
    ...
    getAllOptions(): OptionComponent[] {
        ...
        return this.options.length > 0 ? this.options.toArray() : this.optGroups.map(grp => grp.options.toArray()).flat();
    }
        ...
        if (this.value) {
           ...
           let indexOfChosenGroup = -1;
            if (this.optGroups.length > 0) {
                const groupOfChosenOption = this.optGroups.find(grp => grp.options.toArray().indexOf(chosenOptions[0]) > -1);
